...
const register = async (req, res) => {
  ...
  const rolFinal = rol === 'artesano' ? 'artesano' : 'comprador';
  ...
  try {
    ...
    // Si es artesano, crear perfil vacío
    if (rolFinal === 'artesano') {
      await db.query(
        `INSERT INTO artesanos (usuario_id, nombre_completo) VALUES ($1, $2)`,
        [usuario.id, nombre]
      );
    }
